...
{
    hashTable* vertexTable; // mapping of all intersections (vertexes) in the graph
    Vertex* vertexArray; // list of every intersection (vertex) as it was added to the graph
    int numVertices; // number of intersections (vertexes) in the graph
    int capacity; // maximum allowed intersections (vertexes) in the graph

    /* weights of edges are stored in this matrix (-1 if edge isn't present) */
    int** adjacencyMatrix; // represents difficulty to travel down a road (edge) as a numerical value (-1 if impossible)

    bool changed;           /* check for changes to know whether paths needs to be updated */
    int** pathsArray; // list of next intersection (vertex) to visit on shortest path
    int** distance; // fastest road (edges) between 2 intersections (vertexes) based on distance and weight
    edgeType** edgeData; // stores the data of each road (edge) since they are all different and don't all have a weight of 1
